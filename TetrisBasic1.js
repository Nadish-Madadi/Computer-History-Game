let canvas;
let ctx;
let gBArrayHeight = 40; // Number of cells in array height
...
let speed = 1; // Displays speed of game
...
function SetupCanvas(){
    ...
    ctx.fillText("Speed", 300, 157);
 
    // Draw speed rectangle
    ctx.strokeRect(300, 171, 161, 24);
 
    // Draw speed
    ctx.fillText(speed.toString(), 310, 190);
...
function CheckForVerticalCollison(){
    ...
    if(collision){
        ...
        if(startY <= 2){
            winOrLose = "Game Over"
            document.location.href = "RestartGame1.html";
            ctx.fillStyle = 'black';
